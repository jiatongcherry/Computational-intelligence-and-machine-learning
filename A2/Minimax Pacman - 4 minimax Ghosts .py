...
def minimax(problem, k, num_agents,whichagent, alpha, beta, recent_moves):
    ...
    best_score = -float('inf') if agent == 'P' else float('inf')
    ...
    for direction, position in find_direction(agent, problem).items():
        ...
        next_agent = (whichagent + 1) % num_agents

        result = minimax(new_problem, k - 1, num_agents, next_agent, alpha, beta, recent_moves)
        score = result[0]
...
def min_max_multiple_ghosts(problem, k):
    ...
    while True:
        ...
        if action_list.keys():
            alpha = float('-inf')
            beta = float('inf')
            move = minimax(problem, k,  agent_num, 0,alpha, beta, recent_moves)[1]
        ...
        for ghost in ghost_order:
            if ghostpos[ghost]:
                ...
                if action_list:
                    ...
                    move = minimax(problem, k, agent_num, index[ghost],alpha, beta, recent_moves)[1]
                    #print(move)

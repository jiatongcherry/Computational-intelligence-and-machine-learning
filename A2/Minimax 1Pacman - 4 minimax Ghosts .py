...
def minimax(problem, depth, agent_index, num_agents, alpha, beta, recent_moves):
    if depth == 0 or problem['P'] in [problem['W'], problem['X'], problem['Y'], problem['Z']] or len(problem['food_store']) == 0:
    ...
    #decide whose turn and the initial score
    if agent == 'P':
        best_score = -float('inf')
    ...
    for move, position in find_direction(agent, problem).items():
        ...
        result = minimax(new_problem, depth - 1, next_agent_index, num_agents, alpha, beta, recent_moves)
        score = result[0]
...
def min_max_multiple_ghosts(problem, k):
    ...
    while True:
        ...
        if len(action_list.keys()) != 0:
            alpha = float('-inf')
            beta = float('inf')
            action = minimax(problem, k, 0, agent_num, alpha, beta, recent_moves)[1]

            problem['P'] = (problem['P'][0]+action_list[action][0], problem['P'][1]+action_list[action][1])
        ...
        #ghost round
        ghost_position = {"W": problem['W'], "X": problem['X'], "Y": problem['Y'],
                          "Z": problem['Z']}
        for ghost in ["W", "X", "Y", "Z"]:
            if ghost_position[ghost]:
                turn += 1
                ...
                if len(action_list.keys()) != 0:
                    ...
                    param = {"W": 1, "X": 2, "Y": 3, "Z": 4}
                    action = minimax(problem, k, param[ghost], agent_num, alpha, beta, recent_moves)[1]
                    #print(action)

                    ghost_position[ghost] = (ghost_position[ghost][0]+action_list[action][0], ghost_position[ghost][1]+action_list[action][1])
                    problem['W'], problem['X'], problem['Y'], problem['Z'] = \
                    ghost_position['W'], ghost_position['X'], ghost_position['Y'], ghost_position['Z']


            if problem['P'] == ghost_position[ghost]:
                problem['score'] += PACMAN_EATEN_SCORE
                winner = 'Ghost'
                return solution, winner
